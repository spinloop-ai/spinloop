...
index](../index.md); for how it is required to behave, `openspec/specs/`; for
...
[`AGENTS.md`](https://github.com/spinloop-ai/spinloop/blob/main/AGENTS.md) is
the map — one line per package, each naming the
...
read it at run time with `--providers` or `SPINLOOP_PROVIDERS`. The reasoning
is in [implementation notes](internals.md).

## Adding a harness

Start at the `Harness` interface in `internal/harness`, which holds the adapter
registry for opencode, Pi and lucinate.
[`AGENTS.md`](https://github.com/spinloop-ai/spinloop/blob/main/AGENTS.md) walks
