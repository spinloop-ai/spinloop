...
If `GOOGLE_VERTEX_PROJECT` is unset, `spinloop harness apply` fails with a clear error
naming the option and the variable.

## 2. Apply the Spinloop

```sh
spinloop harness apply examples/google-vertex-anthropic/claude-sonnet/Spinloop
