...
- [x] 1.2 Add `SourceLogger`: reading a node's log by source, time window and
      ...
      not implement it.
- [x] 1.3 Add `Versioner`: the spinloop release a node reports. Verify it is
      answered from the reply the node already holds, with no extra call
      (design D3), and that `statsFromRemote` stops dropping the version and
      instance type.
...
- [ ] 3.3 Signpost all five moved spellings through `movedSubcommands`, giving
...
- [ ] 4.1 Update every invocation of the five moved spellings across `docs/`,
      ...
      spelling.
- [ ] 4.2 Add `docs/commands/metrics.md` and `logs.md`, each saying which
...
- [ ] 4.3 Verify nothing an operator could get from the removed commands is
      unreachable: the cost, the version, the log sources, the `ENV` path, and
      the endpoint address via `remote env`.
- [ ] 4.4 Run `gofmt -l .` (expect no output), `go vet ./...` and
