...
file: the gateway by a required flag, the items file by a flag with a default
of `./work.yaml` in the working directory. It SHALL authenticate to the
gateway with a bearer token resolved the way the fleet file's gateway section
resolves one: a flag naming the environment variable, defaulting to
`OPENAI_API_KEY`. It SHALL NOT take a fleet file: the gateway is its only
...
and concurrency limits only from the gateway's topology: it SHALL NOT hold a
fleet file, a node's token, or an engine key, and it SHALL NOT contact a
node's control API directly. It SHALL re-read the topology as the fleet
changes, so a node that starts, stops, or changes what it serves is seen
