...
- [ ] 1.1 Add a small client helper in `cmd/spinloop` that takes the API's base
...
- [ ] 1.2 Rewrite `work add` to send the item's fields (`id`, `instructions`,
...
- [ ] 1.3 Rewrite `work remove <id>` to call `DELETE /v1/items/{id}` — verify
...
- [ ] 1.4 Rewrite `work abort <id>` to call `POST /v1/items/{id}/abort` —
  verify with a test that asserts the path and the command names the API's
  refusal where the item is not running
- [ ] 1.5 Rewrite `work list` to read `GET /v1/items` and render each returned
...
- [ ] 2.1 Put `--url` (required), `--api-token`, and `--api-token-file` on the
...
- [ ] 2.2 Remove the now-unused file helpers and waits from `cmd/spinloop`
  ...
  the removed symbols
- [ ] 2.3 Confirm the shell completion reflects the new flags and none of the
...
- [ ] 3.1 Rewrite `cmd/spinloop/work_test.go` to work the commands against a
  ...
  or above 80%
- [ ] 3.2 Run the full suite with the race detector — verify with
  `go test ./... -race -cover` passing, no regressions

## 4. Documentation

- [ ] 4.1 Rewrite `docs/commands/work.md` for the API-based commands: the
...
- [ ] 4.2 Fix the cross-references that describe the work commands as working
