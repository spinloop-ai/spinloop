...
- [ ] 1.1 Add a `dashNotServing` tier to the `dashHealthTier` enum, and
...
- [ ] 1.2 In `dashHealthTierFor`, return `dashNotServing` when the node
...
- [ ] 1.3 In `dashHealthGlyph`, render `dashNotServing` as the faded grey
...
- [ ] 2.1 Update `TestDashTileStoppedByteStable` to expect
  `dashHealthGlyph(dashNotServing)` on the `idle` node's name line.
- [ ] 2.2 In the `dashHealthTierFor` table test, move the `idle` row from
...
- [ ] 2.3 Confirm the remaining table rows are untouched: running+ready and
...
- [ ] 3.1 `go test ./...` passes, `go vet ./...` is clean, `gofmt -l ./...`
  prints nothing, and `go test ./... -cover` keeps total coverage >= 80%.
- [ ] 3.2 Check no other surface drew the green for these states:
