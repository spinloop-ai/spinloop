...
- [x] 1.1 Add a helper (e.g. `dashStateLine(state string, m metrics.Stats) string`) in `cmd/spinloop/dashboard_render.go` that returns `state` alone, or `state  (up <duration>)` when `m.UptimeSeconds > 0`, reusing `formatDuration`.
...
- [x] 2.1 Remove the `UptimeSeconds` block from `dashTileServingLine` in `cmd/spinloop/dashboard_render.go`, leaving just runner and model ID.
- [x] 2.2 Update `dashTileServingLine`'s doc comment to describe the new output (runner and model only).

## 3. Update tests and verify

- [x] 3.1 Update `TestDashTileRunningByteStable` in `cmd/spinloop/fleet_dashboard_test.go`: move `(up 2h 0m 0s)` from the serving line onto the `"up  running"` top line, and drop it from the serving line.
...
- [x] 3.3 Update `TestDashGridRealTilesSideBySide` and any other fixture in `cmd/spinloop/fleet_dashboard_test.go` asserting a `(up ...)` line, to match the new layout.
...
- [x] 3.5 Run `gofmt -l cmd/spinloop` and `go test ./cmd/spinloop/... -run Dash -v` to confirm formatting and all dashboard tests pass.
