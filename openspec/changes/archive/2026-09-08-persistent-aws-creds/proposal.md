...
- CDK: `remote/lib/llm-stack.ts` adds the IAM user and its inline policy (the name is a constant on the Go side, not a stack output); existing deployments need a re-bootstrap before `remote auth --store` works.
