# Design: install-spinloop-at-boot
...
- The shared bake preamble (`remote/lib/image-stack.ts`) currently downloads a
  pinned spinloop release, checksum-verifies it against the release's own
  `checksums.txt`, and installs it to `/usr/local/bin/spinloop`; the version is
  the `SpinloopVersion` Image Builder parameter, defaulting to the latest git
...
- The instance boot is one user-data script (`buildInferenceUserData` in
  ...
  `spinloop-daemon.service`, whose `ExecStart` is `/usr/local/bin/spinloop daemon`.
...
- An spinloop-only release reaches environments without an AMI bake: the AMI is
  spinloop-agnostic, and a fresh boot installs the binary.
- The boot install is idempotent, checksum-verified, and atomic, and it runs
  before the daemon's unit is written and enabled.
- An environment may pin an exact spinloop release via `spinloop remote deploy`;
  the default is `latest`, resolved at boot.
- Rollout is safe in both directions: the new boot works on today's
  spinloop-baked AMIs, and today's boot works on the new AMIs' predecessors.

**Non-Goals:**

- No `VERSION`/spinloop keyword in the Spinloop grammar — the pin is deployment
...
An optional `spinloopVersion` field is added to the deploy-config contract
(`DeployConfig` in `lambda/shared/deploy-config.ts`, with `spinloopVersion`
normalised to `"latest"` when absent) and to the Go client's
`remote.DeployConfig` (`SpinloopVersion string` with `omitempty`, so an
...
- Why here: it is already the per-environment document the start Lambda reads
  at every wake and feeds to `buildInferenceUserData`, it is what `spinloop
...
```sh
SPINLOOP_VERSION='<pin, or empty for latest>'
if [ -z "$SPINLOOP_VERSION" ]; then
  SPINLOOP_VERSION="$(curl -fsSL https://api.github.com/repos/spinloop-ai/spinloop/releases/latest \
    | sed -n 's/.*"tag_name": *"\(v\{0,1\}\)\([^"]*\)".*/\2/p' | head -n1)"
fi
test -n "$SPINLOOP_VERSION" || { echo "spinloop version unresolved" >&2; exit 1; }
```
...
- Version strings are the tag minus a leading `v`, matching what the stamped
  ...
  (`releases/download/v$SPINLOOP_VERSION/…`).
...
- Idempotent: the installed binary's own `spinloop version` is compared with the
...
### D4: CLI surface — `--spinloop-version` on `spinloop remote deploy` only

`spinloop remote deploy` gains `--spinloop-version` (long form; the pin is an
...
prints the result as an `spinloop:` line beside the runner and model:
...
- Why deploy-only: the pin is a property of the deployment (like the runner
  ...
  means.
- Alternatives considered: a Spinloop keyword (rejected — the Outfile is
...
- `image-stack.ts`: delete the spinloop block from `commonPreamble()`, the
  `SpinloopVersion` parameter from both component docs and `runnerBuilds`, the
  `spinloopParam`, and the synth-time empty-version guard. Bump `RUNNER_VERSION`
...
- `config.ts`: delete `spinloopVersion` from `LlmConfig`, `DEFAULTS`, and
  `loadConfig`, and `latestReleaseVersion()` with it (its only consumer was
  the guard above).
- `.github/workflows/remote.yml`: delete the `-c spinloopVersion=0.0.0-ci`
  smoke-synth placeholder and its now-stale comment.
- The `spinloop-nudge` script, logrotate config, and CloudWatch agent install
  stay baked — they are instance plumbing, not the spinloop release.
...
- [GitHub outage or API rate-limit at boot breaks a fresh launch] → The
  ...
  operator re-runs `spinloop remote start` once GitHub is reachable. This
...
- [Rollout skew: new boot on old AMI, or old boot on new AMI] → Both are safe
  ...
  hazardous combination (old user data on an spinloop-less AMI) only arises on
...
- [The deploy config is hand-editable SSM state] → A malformed
  `spinloopVersion` in a hand-written config fails `parseDeployConfig` at the
...
1. Ship the control-plane change (Lambda code: deploy-config contract, user
   data with the install step) via `spinloop remote bootstrap`. Current
   spinloop-baked AMIs keep working: fresh launches run the new user data, which
...
3. Bump `RUNNER_VERSION`, `cdk deploy` the image stack, and `pnpm bake` both
   runners' spinloop-less AMIs. The start Lambda picks the newest tagged AMI, so
