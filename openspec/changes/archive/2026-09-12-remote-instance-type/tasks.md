...
- [x] 1.2 Add an exported EC2 instance-type pattern and validator in
...
- [x] 2.1 Add `InstanceType string` (YAML key `instance-type`) to `NodeConfig`
...
- [x] 2.2 In `validate()`, reject `instance-type` on a `kind: daemon` node and
...
- [x] 3.1 Add an `instanceType string` field to `deployOpts` and an
...
- [x] 3.2 In `runDeploy`, when `opts.instanceType` is non-empty, validate it
...
- [x] 3.3 Print the resolved instance type in the deploy plan (the type when
...
- [x] 4.1 In `deployOneNode`, set the targeted node's `InstanceType` into the
...
- [x] 5.1 Add `instanceType?: string` to the `DeployConfig` interface and parse
...
- [x] 5.2 Confirm `writeDeployConfig`/`readDeployConfig` round-trip the field
...
- [x] 6.1 In `remote/lambda/start/index.ts`, launch with
  ...
  the env-var default.
- [x] 6.2 Update the no-capacity reply to name the instance type it was trying
  (rather than a hard-coded `g6e`). Verify the no-capacity test asserts the
  type appears in the message.

## 7. Whole-repo verification

- [x] 7.1 Run `gofmt -l` over the touched Go files and confirm no output; run
  `go vet ./...` and confirm it is clean.
- [x] 7.2 Run `go test ./... -cover` and confirm the suite passes with total
  coverage still >= 80%.
- [x] 7.3 From `remote/`, run `pnpm build` (tsc type-check) and `pnpm test`
  (vitest) and confirm both pass.

## 8. Docs and examples

- [x] 8.1 Document `instance-type` in the `fleet.yaml` reference (remote-only,
...
- [x] 8.2 Add an `instance-type` to a `kind: remote` node in the fleet/remote
