...
- [x] 3.1 Add a `dashHealthTier` type with four values (e.g. `dashHealthy`, `dashAttention`, `dashUnhealthy`, `dashUnknown`) in `cmd/spinloop/dashboard_render.go`.
...
- [x] 5.3 Update `TestDashTileRunningByteStable`, `TestDashTileOutcomeAndEmpty`, `TestDashTileStoppedByteStable`, `TestDashTileActionInFlight`, and `TestDashTileActionInFlightWithReport` in `cmd/spinloop/fleet_dashboard_test.go` to expect the glyph and its colour on each fixture's first line.
...
- [x] 5.6 Run `gofmt -l cmd/spinloop internal/daemon internal/metrics` and `go test ./cmd/spinloop/... -run Dash -v` to confirm formatting and all dashboard tests pass.
