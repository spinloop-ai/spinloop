...
- [x] 1.1 Replace the launch's `--fleet needs a Spinloop` refusal in
      ...
      a gateway.
- [x] 1.2 Verify the refusal still stands where the fleet names no gateway,
      with its existing wording — a launch needs a Spinloop to know which model
      to route. Verify with a test over both fleet files.
- [x] 1.3 Move the gateway helpers `runFleetHarness` calls (the gateway
...
- [x] 1.4 Verify the moved behaviours are unchanged: the provider is named
...
- [x] 1.5 Verify a case neither command has run before: trailing arguments
...
- [x] 2.1 Delete `fleetHarnessCmd`, `runFleetHarness` and `cmdFleetHarness`
      from `cmd/spinloop/fleet.go`, and unregister the subcommand in
      `cmd/spinloop/commands.go`. Verify `spinloop fleet --help` no longer
      lists it.
- [x] 2.2 Signpost the moved spelling on the `fleet` group, per design D3, so
...
- [x] 2.3 Delete `cmd/spinloop/fleet_harness_test.go`, having ported its
      gateway cases in 1.4. Verify no test references the removed functions.

## 3. Consumers

- [x] 3.1 Update `examples/gateway-docker/run-tests.sh` (two invocations, plus
...
- [x] 3.2 Update `examples/gateway-docker/README.md` and any other example
      naming the command. Verify no example still invokes `fleet harness`.
- [x] 3.3 Update `docs/commands/fleet.md` (its "Launching the harness" section
...
- [x] 4.1 Verify a launch that names its Spinloop explicitly no longer picks up
...
- [x] 4.2 Run `gofmt -l .` (expect no output), `go vet ./...` and
      ...
      >= 80%.
- [x] 4.3 Verify the tree has one launch command: grep the command tree for a
