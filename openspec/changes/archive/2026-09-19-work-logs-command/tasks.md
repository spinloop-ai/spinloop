...
- [x] 1.1 Add `workLogsCmd` (`work logs <id>`), `--url`/token flags via
...
- [x] 1.2 An item with no output yet (`log: null`) prints nothing and does
  not fail — verify with a test against a fake API answering `log: null`
- [x] 1.3 An id the API refuses (404, unknown id) fails the command naming
  the id, the way `abort`/`remove` already report a refusal — verify with
  a test against a fake API answering the refusal
- [x] 1.4 Wire `workLogsCmd` into `workCmd`'s subcommands — verify with
  `spinloop work logs --help` listing it and `spinloop work --help`
  listing `logs` among the subcommands (no `--url` completion exists on
  any sibling subcommand to mirror — it names a free-text address, not a
  file or an enum)

## 2. Following

- [x] 2.1 Add `-f`/`--follow` to `workLogsCmd`; unset, behavior is
  unchanged from task 1 — verify with a test asserting the flag defaults
  false and a single read still happens
- [x] 2.2 Add `workLogsInterval` (1s, a package var) and
  ...
  printed each time
- [x] 2.3 Each tick also reads the item's `state` from `GET /v1/items`
  ...
  tick's log
- [x] 2.4 An item that drops out of the list mid-follow (removed) ends the
...
- [x] 2.5 Wire `-f` through `followUntilInterrupted`, matching `fleet logs
...
- [x] 3.1 Run the full suite — verify with `go test ./... -race -cover`
