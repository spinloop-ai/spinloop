...
## 10. The fleet file as the gateway's source

- [x] 10.1 Add the `-f`, `--fleet` flag (the fleet file's usage string and file completion, as on the other fleet commands); where `--gateway` is not given, resolve the fleet file — the flag's path or `./fleet.yaml` — and take the gateway's address from its section, with the section's token variable in force where the token flag was not given; where neither names a gateway, fail before any item is worked, naming the flag and the file; verify with command tests: a section in the working directory's file drives a run against a loopback gateway, a file without a section is refused naming it, the section's token variable is honoured, and neither flag nor file names both ways
- [x] 10.2 Update the docs (the command reference's flags and its "no fleet file" line, the feature guide's "what it takes") and walk the two new spec scenarios against the tests; verify the full suite passes
- [x] 10.3 Resolve the gateway's token the way the file resolves its tokens — the environment first, then the `.env` beside the file (`Config.GatewayToken`, the shared chain re-pointed at the gateway) — wherever the gateway comes from the file; verify with a unit test of the precedence and a command test whose token sits only in the `.env` beside the fleet file

## 11. The agent's working directory

- [x] 11.1 Fix the item's `dir:` being silently ignored: `opencode run` without a directory flag takes its working directory from the PWD variable it inherits — the place the orchestrator was started — rather than the process's cwd, so the child's PWD now carries the item's directory (resolved with `filepath.Abs`) alongside `cmd.Dir`; verify with a dispatch test whose own PWD points elsewhere and a live launch whose session records the item's directory, and record the gotcha in `docs/internals.md`
