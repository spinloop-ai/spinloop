...
`spinloop fleet metrics --watch` is a live-redraw dashboard, not a TUI: read-only, no
selection, no actions. To act on what you are looking at you drop out of it and run
`spinloop fleet start <node>`. That is backwards for the case that matters most: the
...
them (spinloop-ai/spinloop#59).

## What Changes

- New `spinloop fleet dashboard` subcommand: a genuine interactive TUI over the fleet.
...
- Selection (j/k and the arrow keys) and keyboard-driven start/stop of the selected
  ...
  to a follow-up (spinloop-ai/spinloop#119).
...
- New runtime dependencies: `charmbracelet/bubbletea` and `lipgloss` —
  spinloop's first UI framework — plus `charmbracelet/x/ansi` (ANSI-aware line
...
- `fleet-client`: gains the `spinloop fleet dashboard` requirements — an interactive
...
- `cmd/spinloop`: new files for the command, the tea model, and the tile/frame
