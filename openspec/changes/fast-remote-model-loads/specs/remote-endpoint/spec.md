...
`spinloop remote start` SHALL accept an explicit pre-warm choice — enabled or
...
- **WHEN** the user runs `spinloop remote start` without a pre-warm choice
...
- **WHEN** the user runs `spinloop remote start` with the pre-warm disabled
