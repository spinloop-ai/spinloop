...
- [ ] 1.1 Extract `resolvePlan` from `Dispatcher.Launch`: the directory
  ...
  refactor alone)
- [ ] 1.2 Introduce the `Launcher` interface (`Launch(Item, Node, string)
...
- [ ] 1.3 Change `WorkList`'s dispatcher field to hold a `Launcher`, `NewWorkList` and `NewDispatcher`'s
  callers updated — verify with `go build ./...` and the orchestrator
  suite passing

## 2. Render a provider config without touching the host

- [ ] 2.1 Add `RenderProviderConfig` to `harness.Harness`, implemented for
...
- [ ] 2.2 Lucinate's harness is left without the method (it has no
...
- [ ] 3.1 Add a `harness.yaml` loader: `--harness-config`, else
...
- [ ] 3.2 Fold `harness.yaml`'s `env` into `resolvePlan`'s environment; an
...
- [ ] 3.3 Add the wrapper: a generated POSIX shell script (design.md's
...
- [ ] 3.4 Wire the wrapper into the bare backend: `startChild` runs the
...
- [ ] 3.5 A wrapper exit of 97 is read as "startup failed", not the
...
- [ ] 3.6 An abort of a wrapped item: `Stop`/`Kill` act on the wrapper,
...
- [ ] 4.1 Add `dockerLauncher` implementing `Launcher`: calls
...
- [ ] 4.2 Build the `docker run -d` invocation: `--network host`, the
...
- [ ] 4.3 Implement `dockerChild`: `Wait` as `docker wait`, `Stop` as
  ...
  does
- [ ] 4.4 A docker failure (the run command's non-zero exit, or the
...
- [ ] 5.3 Wire a CI job that builds and publishes the image on a tag,
...
- [ ] 6.1 Add `--dispatch` to `spinloop orchestrator` (`bare` default,
...
- [ ] 6.2 Where `--dispatch docker` is given, run `docker version` once
...
- [ ] 6.3 Add `--dispatch-image`, defaulting to
...
- [ ] 6.4 Add `--harness-config`, naming the file §3.1's loader reads —
  verify with a test asserting the flag's file is read in preference to
  a `harness.yaml` beside the items file

## 7. Cleanup

- [ ] 7.1 `work remove` and the state's own end-of-run cleanup take the
...
- [ ] 8.1 Document `--dispatch`, `--dispatch-image`, `--harness-config`,
...
- [ ] 8.2 A short README under `images/agent/` describing what the image
...
- [ ] 9.1 Run the full suite with the race detector — verify with `go
