...
- [x] 1.1 Create `internal/hf` (stdlib only, plus `internal/contextsize`) with a
  `Ref` type holding repo owner, name, revision, and an optional quant or file.
- [x] 1.2 Implement `ParseRef`: bare `org/model`, `hf.co/`/`huggingface.co/`
  prefixes, full `https://huggingface.co/...` URLs with `/tree/<rev>` and
  `/blob/<rev>/<file>`, `:QUANT` suffixes, and `@<revision>` suffixes.
- [x] 1.3 Reject a reference naming no organisation, and one whose `@revision`
  contradicts a `/tree/<rev>`, with messages naming what was wrong.
- [x] 1.4 Table-driven tests covering every accepted form and each rejection.

## 2. Hub client

- [x] 2.1 Resolve the endpoint from `HF_ENDPOINT`, defaulting to
  `https://huggingface.co`; give the client a bounded timeout and no retries,
  following `internal/discovery`'s shape.
- [x] 2.2 Fetch repo metadata (`/api/models/{repo}/revision/{rev}`), reading
  `siblings[].rfilename`, `tags`, `library_name` and `sha`.
- [x] 2.3 Fetch `config.json` via `/{repo}/resolve/{rev}/config.json` and read
...
- [x] 2.4 Resolve an optional token: `HF_TOKEN`, then `HUGGING_FACE_HUB_TOKEN`,
  then `$HF_HOME/token` (else `~/.cache/huggingface/token`); send it as a bearer
  only when one was found.
- [x] 2.5 Map failures to distinct, named errors: not found, gated/private (with
...
- [x] 2.6 Tests against an `httptest` server with `HF_ENDPOINT` pointed at it:
...
- [x] 3.1 Resolve cache roots in one function — `HF_HUB_CACHE`, else
...
- [x] 3.2 Look a ref up in the Hugging Face cache:
  ...
  cached.
- [x] 3.3 Read the cached `config.json` from the snapshot when present, so a
  fully cached model resolves with no network request.
- [x] 3.4 Look a chosen quant up in llama.cpp's cache: a case-insensitive scan
...
- [x] 3.5 Tests over temp-dir caches: a hit in each cache, a dangling snapshot
...
- [x] 4.1 Infer the provider: any `.gguf` → `llamacpp`; else `library_name`/tag
...
- [x] 4.2 Parse quant names from the file list: strip `.gguf` and any
...
- [x] 4.3 Select the quant: reference suffix, then `-q`, then the documented
...
- [x] 4.4 Derive the alias from the repo name: lower-cased, with a trailing
  `-GGUF`/`-MLX` packaging suffix removed.
- [x] 4.5 Build the `spinloop.Selection`: provider, the `MODEL` per the design's
  engine table (cached path for `llamacpp`, repo reference otherwise), alias,
  context from the declared window, and no `OUTPUT`.
- [x] 4.6 Return the reasoning alongside the selection (provider and why, quant
...
- [x] 4.7 Tests for each inference rule, including a repo carrying both GGUF and
  safetensors, a sharded quant, a repo declaring no window, and a repo matching
  no provider.

## 5. The command

- [x] 5.1 Add `cmd/spinloop/hf.go` with `hfCmd()`: flags `-p/--provider`,
...
- [x] 5.2 Render the selection with `spinloop.Format` to stdout; write the
  reasoning to stderr.
- [x] 5.3 Implement `-o`: write the rendered Spinloop to the path, report where it
  went, and refuse to overwrite an existing file unless `--force` is given.
- [x] 5.4 Implement `--apply`: route the resolved selection through
  `applySelection`, honouring `--harness`/`-H`.
- [x] 5.5 Fail with usage when no reference is given.
- [x] 5.6 Tests in `cmd/spinloop/hf_test.go` against a stub Hub and temp caches:
...
- [x] 6.1 Name the command's value completions the way `addCmd` does —
...
- [x] 6.2 Write `docs/commands/hf.md`: the reference forms, each inference and
...
- [x] 6.3 Link the new page from `docs/README.md`, document `HF_TOKEN`,
  ...
  `README.md`.
- [x] 6.4 Add an `internal/hf` entry and a `cmd/spinloop/hf.go` note to
...
- [x] 6.5 Run `gofmt -w ./...`, `go vet ./...` and `go test ./... -cover`,
