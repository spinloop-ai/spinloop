...
`spinloop harness apply`, `spinloop harness unapply`, and `spinloop harness export`.
...
UPPERCASE keywords with aligned values, so `spinloop harness export > Spinloop` round-trips.
